...
## 3) Serve model (higher-throughput profile)

This variant is more aggressive than the baseline:
- enables `fp8` KV cache
- raises concurrency to `2`
- enables prefix caching
- increases batching with `--max-num-batched-tokens 8192`

Use it after the baseline command is stable on your Thor.
